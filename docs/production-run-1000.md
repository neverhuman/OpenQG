...
## Champion `thy-xcde1` — a theory that beats ΛCDM on the data
...
**ε = +36.7** — it fits the combined Tier-0 data **36.7 log-likelihood units better than the GR
ΛCDM baseline** (ε is measured as Δlog-likelihood vs that baseline). This is a real reflection of
the DESI BAO preference for evolving dark energy (w0 ≳ −1) combined with the CMB-prior + BBN
constraints — recovered here through a *derived*, structurally-consistent theory rather than a fit.
...
- **Real forward model**: coverage **1.0** (predicts all 15 observables); ε = +36.7 vs GR.
...
- **Background-only forward model.** The 15 observables are exactly computable from the FLRW
  ...
  Planck-C_ℓ analysis. The ε = +36.7 improvement is driven mainly by the background expansion /
  distance sector (the DESI w0 preference), which the model derives exactly.
