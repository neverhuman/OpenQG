# OpenQG / ZYAL — next-level engineering review (spec S01 of 12)
...
parallel (S01–S12); yours is S01. The team is explicitly willing to make profound changes under the
...
## Your lens: mathematician / formal-verification engineer — INVENT the derivation sandbox

Today, "derivation" means: the proposer cites one of 9 closed-form registry relations
(`crates/openqg-core/src/theory/certificate.rs`, e.g. `ndgp_geff_over_g(β) = 1 + 1/(3β)`), supplies
named numeric inputs, and a pure oracle recomputes the output within tolerance. Rigor weights are
HAND-ASSIGNED per relation (0.0–1.0). That is honest but shallow: it verifies arithmetic, not
derivation. Your charter is to design the V8 derivation sandbox — the machinery that makes
"derived, not fit" mean something a mathematician would sign:

1. **The derivation-trace format.** Spec a step-by-step, machine-checkable derivation artifact: a
   proposal submits a trace from declared axioms/assumptions to the target relation, each step
   tagged with its justification (algebraic rewrite, substitution, limit, approximation with stated
   error bound). Define the step grammar (JSON schema), the Rust-side `DerivationSandbox` trait the
   oracle calls, and the verdict taxonomy (verified / step-N-fails / unjustified-approximation).
2. **The checker stack.** Which technologies verify which step kinds: CAS (SymPy, Symbolica, egg
   e-graph rewriting), validated numerics/interval arithmetic, Schwartz–Zippel-style randomized
   evaluation at random parameter points to test symbolic identities numerically, dimensional
   analysis. Be concrete about sandboxing (separate process, no network, deterministic seeds) and
   about failure attribution.
3. **Rigor computed, not assigned.** Replace hand-assigned rigor weights with a function of the
   verified trace: depth from axioms, assumption count and strength, fraction of steps
   symbolically vs only numerically verified, approximation error budget. Give the formula and a
   gaming-resistance analysis (how does a proposer inflate it, and what kills that?).
4. **A dimensional type system.** Every quantity carries units/mass-dimension through the whole
   trace and into certificate inputs; spec the type system and where it plugs into the existing
   dimensional-homogeneity veto (`vetoes.rs`).
5. **Proof assistants, honestly.** Assess Lean 4 + mathlib feasibility for this domain in 2026:
   which step classes are formally provable today, where the frontier is, hybrid designs
   (CAS-found/Lean-checked certificates), and the fallback discipline for steps that cannot be
   formally verified. Cite real prior art (e.g. AI-Descartes/AI-Hilbert axiom-grounded discovery,
   certified rewriting work).
6. **Invent beyond the obvious.** Propose at least two NEW sandbox techniques, e.g.:
   derivation-by-construction (the sandbox exposes only verified building blocks and the LLM may
   only compose them — soundness by construction); mutation testing of the checker (perturb a step,
   require the checker to catch it — the checker's own regression suite); adversarial
   cross-examination where a second model attacks a trace and the deterministic host referees.
7. **Anti-laundering kill rule.** A numeric posterior median dressed up as a "derivation input"
   must die. Define the rule that separates derivation from disguised fitting, and its test.
8. **Migration + acceptance.** The 9 registry relations re-expressed as verified traces; the
   scorecard's derivation-rigor dimension recomputed from the sandbox; the acceptance test suite
   that proves the sandbox rejects each historical exploit class.

Read first, in order: `crates/openqg-core/src/theory/certificate.rs`,
`crates/openqg-core/src/theory/obligation.rs`, `crates/openqg-core/src/theory/binding.rs`,
`crates/openqg-core/src/theory/scorecard.rs` (rigor weighting), `docs/zyal-next-level-design.md`,
`paper/main.tex` (scoring + limitations).

## Output contract (READ CAREFULLY — this determines whether your work is captured)

Produce your spec as a SINGLE Markdown file named exactly `spec-S01-derivation-sandbox.md`, and
