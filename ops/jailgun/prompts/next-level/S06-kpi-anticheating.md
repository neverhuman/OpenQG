# OpenQG / ZYAL — next-level engineering review (spec S06 of 12)
...
parallel (S01–S12); yours is S06. The team is explicitly willing to make profound changes under the
...
## Your lens: metrology critic — KPIs that measure theory, not fitting

The scorecard (`scorecard.rs`) awards 100 points: derivation rigor 20, data fit 20 (one-sided
ΔlnZ vs ΛCDM), novel prediction 20, unification 15, robustness 13, parsimony 12 — with a veto
cascade in front (`vetoes.rs`). Four champion generations gamed four different dimensions. The team
cares about THEORY, not curve-fitting, and wants the metric system redesigned so that what gets
measured is what actually matters. Your charter:

1. **Gaming-resistance audit, dimension by dimension.** For each rubric dimension, reconstruct how
   it has been or could be gamed. The paper's audit taxonomy (instrument, laundering,
   specification, coherence, economics, friction) is your map — for each dimension, derive the
   exploit PATTERN and judge whether the current fix is structural or a patch awaiting the next
   variant. Where it is a patch, design the structurally immune version.
2. **Should data fit score points at all?** Argue the redesign seriously: evidence as a GATE
   (pass/fail at a pre-registered threshold) with zero points; points reserved for OUT-OF-FIT-SET
   performance only (a time-stamped prediction registry scored on later-released data); 
   multiplicative rigor×novelty scoring so an underived fit earns nothing; profundity weighting
   (10 points earned on data the proposer could not have seen beats 20 on the fit set). Spec the
   rubric you would actually ship, with point allocations and the migration path for comparability
   with V4–V7 scores.
3. **Engine-level KPIs, not just theory-level.** The product is the ENGINE; score it as such.
   Define operationally: time-to-invalidate (adversary-hours a champion survives — make it a
   first-class published number), exploit discovery rate and regression-corpus growth,
   search-volume covered (needs a measure over theory space — propose one), exclusion strength
   delivered (which mechanism classes are ruled out, at what strength), and reproducibility cost
   (CPU-hours to re-verify a campaign from ledgers). Spec the dashboard and where each number
   comes from.
4. **The profound-vs-incremental line.** Define the measurable thresholds the project should aim
   at and publish: what result, in numbers, constitutes "profound" (a pre-registered novel
   prediction confirmed at >Nσ on data released after registration; a mechanism class excluded at
   stated strength over a declared search volume)? Make the scoreboard legible to an outside
   physicist in one glance.
5. **Completeness of the pricing ledger.** k_eff currently prices free parameters, drifted
   background coordinates, and proposer-chosen certificate inputs. What is still UNPRICED:
   tolerance choices, lane selection, prompt content, the novelty-witness tolerance, the choice of
   which observables to claim? For each unpriced DOF: price it or prove it harmless.
6. **Oracle overfitting — the meta-problem.** The rubric has been tuned to kill known exploits;
   that is training on the test set. Spec the held-out audit protocol: reserved exploit classes
   never used for tuning, periodic red-team campaigns with fresh eyes (LLM or human), and the
   statistic that would detect rubric overfit.
7. **Cost-normalized KPIs.** Score-per-token and kills-per-token belong on the dashboard (token
   accounting is S12's charter — assume the ledger exists, define the KPIs).

Read first, in order: `crates/openqg-core/src/theory/scorecard.rs`,
`crates/openqg-core/src/theory/vetoes.rs`, `crates/openqg-core/src/theory/league.rs`,
`crates/openqg-core/src/theory/evidence.rs`, `crates/openqg-core/src/scoring/evidence.rs`,
`paper/main.tex` (scoring + audit-cascade sections), `docs/V6-ACCEPTANCE.md`.

## Output contract (READ CAREFULLY — this determines whether your work is captured)

Produce your spec as a SINGLE Markdown file named exactly `spec-S06-kpi-anticheating.md`, and
