...
## Your lens: quantum gravity & unification (what is worth chasing, and is it falsifiable)

Emphasize the BIG QUESTION the project name ("openQG") implies. What does "the unified theory most
worthy of attention" mean OPERATIONALLY for a data-driven engine? Pure-QG programs — string landscape,
loop quantum gravity, causal dynamical triangulations, asymptotic safety, causal sets — mostly make no
discriminating cosmological prediction. Is the project's implicit verdict correct (admit them only via
falsifiable low-energy effective handles: a running Λ, a primordial-spectrum feature, a bounce
signature, fixed α_i deviations, a specific tensor-to-scalar / non-Gaussianity, Lorentz-violation
bounds)? If so, what ARE the strongest such handles, and which QG program currently offers the most
testable one? Which frontier (inflation alternatives / early universe, the dark sector, stochastic GW
backgrounds, equivalence-principle and Lorentz tests) offers the best ratio of fundamental significance
to "scoreable on attainable data"? Lay out a concrete 2–5 year scoreable program: the single
unified-physics target most worth chasing, the observables that would move belief, and how the engine
should encode and score it without fooling itself.
